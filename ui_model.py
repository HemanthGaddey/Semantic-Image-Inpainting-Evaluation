...
class ui_model(QtWidgets.QWidget, Ui_Form):
    ...
    def showImage(self, fname):
        ...
        value = self.comboBox.currentIndex()+1
    ...
    def set_input(self,text):
        ...
        value = self.comboBox.currentIndex()+5
        ...
        if value < 4:
            ...
            mask = task.random_regular_mask(img).unsqueeze(0)
    ...
    def fill_mask(self,text,name):
        """Forward to get the generation results"""
        img_m, img_c, img_truth, mask, text_idx, text_len = self.set_input(text=text)
        # print('Nee Yabba', img_m)

        # Assuming you have a tensor named 'tensor_image'
        # Convert the tensor to a NumPy array
        # numpy_image = mask.cpu().numpy()

        # # Assuming 'numpy_image' is in the shape (channels, height, width)
        ...
        val=self.comboBox.currentIndex()+1
        if val == 0:
            return
        print("*")
        if text_len < 1:
        ...
        if self.PaintPanel.iteration < 100:
            ...
            with torch.no_grad():
                ...
                img=Image.fromarray(util.tensor2im(self.img_out.detach()))

                # numpy_img=self.img_out.numpy()
                # img_to_save=Image.fromarray(numpy_img)
                folder='/home/hemanthgaddey/Documents/tdanet_/tdanet/train_data/'+'('+str(self.counter)+')'+name.split('/')[-1]
                self.counter+=1
...
#import ipdb; ipdb.set_trace()   #debug line (hemanth)
